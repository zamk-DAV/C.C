...
const DraggableEvent: React.FC<DraggableEventProps> = ({ event, onDrop, onClick, onDrag }) => {
    ...
    const bindLongPress = useGesture({
        onPointerDown: (state: any) => {
            ...
            // Manual Long Press Implementation
            // Clear any existing timeout
            if ((window as any).longPressTimeout) clearTimeout((window as any).longPressTimeout);

            const startX = state.xy[0];
            const startY = state.xy[1];
            ...
            // Store start pos to cancel on move
            (window as any).longPressStartPos = { x: startX, y: startY };
        },
        onPointerMove: (state: any) => {
            const startPos = (window as any).longPressStartPos;
            if (startPos) {
                const moved = Math.abs(state.xy[0] - startPos.x) > 10 || Math.abs(state.xy[1] - startPos.y) > 10;
                if (moved) {
